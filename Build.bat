...
set finalDir=%localDirectory%Final
...
if "%~1" neq "" 
(
    set buildPlatformConfiguration=%~1
)

echo Making Classroom Learning Partner: %buildPlatformConfiguration% Builds.
echo.
echo Building x86...
%msbuildexe% "Classroom Learning Partner.sln" /m /p:Configuration=%buildPlatformConfiguration% /p:Platform="x86" /v:q /t:rebuild
echo Building x64...
%msbuildexe% "Classroom Learning Partner.sln" /m /p:Configuration=%buildPlatformConfiguration% /p:Platform="x64" /v:q /t:rebuild
echo Building AnyCPU...
%msbuildexe% "Classroom Learning Partner.sln" /m /p:Configuration=%buildPlatformConfiguration% /p:Platform="AnyCPU" /v:q /t:rebuild
if not "%ERRORLEVEL%"=="0" 
(
    echo ERROR: Build Failed. & goto Quit
)
echo.
