...
namespace CLP.Models
{
    ...
    public class CLPAggregationDataTable : CLPPageObjectBase, ISubmittable
    {
                    ...
                    case GridPartOrientation.Row:
                        ...
                        if (replaceIndex > -1)
                        {
                            var strokesToRemove =
                                ...
                                from objectStroke in StrokeDTO.LoadInkStrokes(Rows[replaceIndex].SerializedStrokes)   //CLPPage.BytesToStrokes(Rows[replaceIndex].ByteStrokes)
                            ...
                            {
                                ParentPage.AddInkWithoutHistory = true;
                                ParentPage.InkStrokes.Remove(s);
                                ParentPage.AddInkWithoutHistory = false;
                            }
                        ...
                        StrokeCollection gridPartStrokes = StrokeDTO.LoadInkStrokes(gridPart.SerializedStrokes);            //.BytesToStrokes(gridPart.ByteStrokes);
        ...
        public CLPAggregationDataTable CreateAggregatedTable(CLPGridPart gridPart)
        {
            ...
            switch(gridPart.Orientation)
            {
                case GridPartOrientation.Row:
                    ...
                    {
                        var newCol = new CLPGridPart(col.Orientation, RowHeaderWidth ,col.Width);
                ...
                case GridPartOrientation.Column:
                    foreach(CLPGridPart row in Rows)
                    {
                        var newRow = new CLPGridPart(row.Orientation, row.Height, ColumnHeaderHeight);
            ...
            if(CanAcceptStrokes)
            {
                foreach(CLPGridPart gridPart in Rows)
                {
                    if(gridPart.IsAggregated)
                    {
                        ...
                        gridPart.SerializedStrokes = StrokeDTO.SaveInkStrokes(clonedStrokes);
                    }
                }
            }
        }
        ...
        public void AfterSubmit(bool isGroupSubmit, CLPNotebook notebook)
        {
            ...
            ICLPPage autoGeneratedPage = notebook.GetPageAt(autoPageIndex, -1);
