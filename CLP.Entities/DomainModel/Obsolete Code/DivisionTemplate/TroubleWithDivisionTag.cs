...
namespace CLP.Entities
{
    ...
    public class TroubleWithDivisionTag : ATagBase
    {
        ...
        public override string FormattedName => "Trouble With Division";
            ...
            get
            {
                return
                    string.Format("{0}{1}{2}{3}{4}{5}",
                                  GetTroubleWithFactorPairsCount(ParentPage) == 0 ? string.Empty : string.Format("Trouble with Factor Pairs {0} time(s).\n", GetTroubleWithFactorPairsCount(ParentPage)),
                                  GetTroubleWithFactorPairsCount(ParentPage, true) == 0 ? string.Empty : string.Format("{0} deleted.\n", GetTroubleWithFactorPairsCount(ParentPage, true)),
                                  GetTroubleWithRemaindersCount(ParentPage) == 0 ? string.Empty : string.Format("Trouble with Remainders {0} time(s).\n", GetTroubleWithRemaindersCount(ParentPage)),
                                  GetTroubleWithRemaindersCount(ParentPage, true) == 0 ? string.Empty : string.Format("{0} deleted.\n", GetTroubleWithRemaindersCount(ParentPage, true)),
        ...
        {
            var totalCount = page.Tags.Count(tag => tag is DivisionTemplateFactorPairErrorsTag && (tag as DivisionTemplateFactorPairErrorsTag).HadTrouble);
        ...
        {
            var totalCount = page.Tags.Count(tag => tag is DivisionTemplateRemainderErrorsTag && (tag as DivisionTemplateRemainderErrorsTag).HadTrouble);
        ...
        public static int GetTroubleWithDivisionTemplateCreationCount(CLPPage page, bool returnOnlyDeleted = false)
        {
            ...
            var deletedCount = page.Tags.Count(tag => tag is DivisionTemplateCreationErrorTag && !(tag as DivisionTemplateCreationErrorTag).IsDivisionTemplateStillOnPage);
