...
{
    public abstract class APageObjectAccepter : APageObjectBase, IPageObjectAccepter
    {
        #region Constructors

        protected APageObjectAccepter() { }
        ...
        public static void SplitAcceptedPageObjects(List<IPageObject> oldPageObjects, List<IPageObject> newPageObjects, bool isPageObjectSingleCapture = true)
        {
            foreach (var oldPageObject in oldPageObjects)
            {
                var pageObjectAccepter = oldPageObject as IPageObjectAccepter;
                if (pageObjectAccepter == null)
                {
                    continue;
                }

                foreach (var acceptedPageObject in pageObjectAccepter.AcceptedPageObjects)
                {
                    var validPageObjectAccepters =
                        newPageObjects.OfType<IPageObjectAccepter>()
                                      .Where(p => (p.CreatorID == pageObjectAccepter.CreatorID || p.IsBackgroundInteractable) && p.IsPageObjectOverThisPageObject(acceptedPageObject))
                                      .ToList();
                    if (isPageObjectSingleCapture)
                    {
                        IPageObjectAccepter closestPageObject = null;
                        foreach (var pageObject in validPageObjectAccepters)
                        {
                            ...
                            if (closestPageObject.PercentageOfPageObjectOverThisPageObject(acceptedPageObject) < pageObject.PercentageOfPageObjectOverThisPageObject(acceptedPageObject))
                            {
                                closestPageObject = pageObject;
                            }
                        }
